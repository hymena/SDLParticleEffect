...
namespace farukprogramming{
    ...
    bool Screen::init(){
        ...
        m_buffer2 = new Uint32[SCREEN_HEIGHT*SCREEN_WIDTH]; // allocating space for buffer
        memset(m_buffer,0,SCREEN_WIDTH*SCREEN_HEIGHT*sizeof(Uint32)); // filling the buffer with 0
        memset(m_buffer2,0,SCREEN_WIDTH*SCREEN_HEIGHT*sizeof(Uint32)); // filling the buffer with 0
    ...
    void Screen::boxBlur() {
        //Swap the buffers, so pixel info is in buffer2 and we are drawing to m_buffer1
